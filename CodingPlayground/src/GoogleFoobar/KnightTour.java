...
/*
 * As a henchman on Commander Lambda's space station, you're expected to be resourceful, smart, and a quick thinker. It's not easy building a doomsday device and 
 * ordering the bunnies around at the same time, after all! In order to make sure that everyone is sufficiently quick-witted, Commander Lambda has installed new 
 * flooring outside the henchman dormitories. It looks like a chessboard, and every morning and evening you have to solve a new movement puzzle in 
 * order to cross the floor. That would be fine if you got to be the rook or the queen, but instead, you have to be the knight. 
 ...
 * and the destination square, which is where you need to land to solve the puzzle.  The function should return an integer representing the smallest number of 
 * moves it will take for you to travel from the source square to the destination square using a chess knight's moves 
